...
static void unselect_rows(void) {
    ...
    for(uint8_t x = 10; x < MATRIX_ROWS; x++) {
...
static void select_row(uint8_t row) {
    ...
    switch (row) {
        case 0:
            send_data[0] = 0xFE;
            send_data[1] = 0xFF;
            Expander_Adr = PORT_EXPANDER0_ADDRESS;
            break;
        case 1:
            send_data[0] = 0xFD;
            send_data[1] = 0xFF;
        ...
        case 5:
            send_data[0] = 0xFE;
            send_data[1] = 0xFF;
            Expander_Adr = PORT_EXPANDER1_ADDRESS;
            break;
        case 6:
            send_data[0] = 0xFD;
            send_data[1] = 0xFF;
    ...
    if (Expander_Adr > 0) {
        i2c_writeReg((Expander_Adr << 1), 0x00, &send_data[0], 2, 20);
    }
...
static void init_pins(void) {
    ...
    for (uint8_t x = 12; x < MATRIX_COLS; x++) {
